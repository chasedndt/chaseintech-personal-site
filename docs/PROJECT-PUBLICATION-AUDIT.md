...
| Toolshape Studio | Public `chasedndt/toolshape-studio` architecture pack and validation report | Planned architecture/product definition; validation covers the pack, not a working editor | `toolshape-studio-semantic-kernel-before-ui` published and linked |
| Toolshape Voice | Public `chasedndt/toolshape-voice` repository is currently empty | Planned publicly; working-build, integration and packaging claims withheld pending fresh publishable evidence | Earlier launcher log returned to draft and unlinked |
